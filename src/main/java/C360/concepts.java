...
class concepts extends C360_GUI{
    ...
    try { 
        ...
        // Pagination    ????
        Element_E1_Find("Find Concepts Table body", "tagName", "tbody", ParentTest, "no_jira");
             if (FAIL) { return;}                     
        Element_Child_List_L1("Concepts Table Rows", e1, "tagName", "tr", ParentTest, "no_jira");
            for (int i = 0; i < L1.size(); i++) {
                Element_Text("Get Concepts Table Row " + (i + 1) + " concatenated text", L1.get(i), ParentTest, "no_jira"); 
                if(!_All_data) { break;}
            }
