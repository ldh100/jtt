...
class users extends C360_GUI{
    ...
    try { 
        ...
        Click_out_of_Element("Click down of the 'Header Bar' - activate email check", e1, "DOWN", 0, 4, ParentTest, "no_jira");
             if (FAIL) { return;}        
        Element_By_Path_Text("Find 'Enter Email error' text", "xpath", "//span[@class='caption']", ParentTest, "no_jira"); 
        
        Element_By_Path_Input_Select_Clear("Clear invalid Emailt", "xpath", "//input[@placeholder='Search Email']", ParentTest, "no_jira"); 
        Element_By_Path_Text_Enter("Enter invalid Email", "xpath", "//input[@placeholder='Search Email']", "a@b.com", false, ParentTest, "no_jira"); 

        
        Element_Child_Click("Click 'Discard' button", e1, "xpath", ".//span[text()='Discard']/ancestor::button", ParentTest, "no_jira");
             if (FAIL) { return;}
        Element_By_Path_Text("Find 'Dialog Header' text", "xpath", "//ispan[@class='p-dialog-title']", ParentTest, "no_jira"); 
        Element_By_Path_Text("Find 'Dialog Message' text", "xpath", "//span[@class='p-confirm-dialog-message']", ParentTest, "no_jira"); 

        //Element_By_Path_Text("Find 'Dialog Reject' text", "xpath", "//buttonn[@class='p-button p-component p-confirm-dialog-reject p-button-text']", ParentTest, "no_jira"); 
