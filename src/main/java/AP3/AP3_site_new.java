...
class AP3_site_new extends AP3_GUI{
    ...
    try {    
        Move_to_Element_By_Path("Open Dashboard Drawer", "xpath", "//aside[contains(@class, 'navigation-drawer')]", ParentTest, "no_jira");             
            if (FAIL) { return;}
        Thread.sleep(500);          
        Element_By_Path_Click("Drawer > Click 'Sites'", "xpath", "//*[contains(text(), 'Sites')]", ParentTest, "no_jira"); 
            if (FAIL) { return;} 
            ...
            switch (CHOICE) { 
                case "Station Information":            
                    ...
                    if (T_Index > -1) {
                        ...
                        Element_Click("Click 1st listed Group (" + SECTOR + " not found)", L1.get(0), ParentTest, "no_jira");    // === invisible DEBUG
                            if (FAIL) { return; }
                    }
                    // ========================================= Group Selection ^^^^
                ...
                case "Delivery Details":     
                   ...
                   List_L1("Input 'Timeslot Length' Count", "css", "[aria-label='Timeslot Length']", ParentTest, "no_jira"); 
                    ...
                    Element_Text_Enter("Enter Delivery 'Customers Per Slot'", L1.get(1), "3", ParentTest, "no_jira"); // 2nd
                        if (FAIL) { return;}  
                    Element_By_Path_Click("Click 'Choose Drop-off Locations'", "xpath", "//a[contains(text(), 'Choose Drop-off Locations')]", ParentTest, "no_jira"); 
                       if (FAIL) { return;}
                   Element_E1_Find("Find 'Choose Drop-off' dialog", "xpath", "//div[@class='v-dialog v-dialog--active']", ParentTest, "no_jira");
                       if (FAIL) { return;}         
                   Element_Child_Click("Click 'Select All'", e1, "xpath", ".//div[contains(@class,'v-input--selection-controls__input')]", ParentTest, "no_jira"); 
                       if (FAIL) { return;}   
                    Element_Child_List_L1("Get 'Choose Drop-off' dialog action buttons", e1, "xpath", ".//button[@type='button']", ParentTest, "no_jira");  
                       if (FAIL) { return;}                     
                    Element_Click("Click 2nd button > 'Continue'", L1.get(1), ParentTest, "no_jira"); 
                       if (FAIL) { return;}    
                     break;
                case "Scan & Go Setup":  
...
//                        if (FAIL) { return;} 
                ...
                case "Web Ordering":  
                    ...
                    Element_Child_List_L1("Find 'Allow Web Ordering' options", e1, "xpath", ".//div[contains(@class, 'Option-Left')]", ParentTest, "no_jira");             
                        if (FAIL) { return;} 
                    Element_Click("Click 'Allow Web Ordering' - Yes", L1.get(1), ParentTest, "no_jira");
                        if (FAIL) { return;}                   
                    break;
                default:
                    break;
...
//            if (FAIL) { return;} 
        ...
        List_L0("Tabs Count", "xpath", "//div[contains(@class, 'v-tabs__div')]", ParentTest, "no_jira");             
            if (FAIL) { return;}    
         for (int i = 0; i < L0.size(); i++) {     
            Element_Text("New Brand Setup Tab " + (i+1) + " >>>>", L0.get(i), ParentTest, "no_jira"); 
         }
...
//                }                  
        ...
        Element_By_Path_Text_Enter("Enter Local Menu Search", "css", "input[aria-label='Search Local Menus']", New_ID, false, ParentTest, "no_jira");
            if (FAIL) { return;} 
        Thread.sleep(1000);  
...
//                    if (FAIL) { return;}                     
             ...
             for (int i = 0; i < L0.size(); i++) { // ========================================================================================
                ...
                switch (CHOICE) { 
                    ...
                    case "Scan & Go Setup":  
                        if(flag == 2) {
                            Element_E1_Find("Find 'Enable Scan & ...'", "id", "toc-ScanAndGo", ParentTest, "no_jira"); 
                                if (FAIL) { return;}
                            Element_Child_List_L2("Find 'Enable Scan & Go Setup' options", e1, "xpath", ".//div[contains(@class, 'Option-Left')]", ParentTest, "no_jira");             
                                if (FAIL) { return;} 
                            Element_Click("Click 'Enable Scan & Go' - Yes", L2.get(0), ParentTest, "no_jira");
                                if (FAIL) { return;}                                                       
                        }
                        break;
                    case "Assign Menus":
                        if(flag < 2) {
                            Find_Text("Find 'Menu for unit will be auto-assigned...'", "Menu for unit will be auto-assigned", true,ParentTest, "no_jira"); 
                        } else {
                            List_L3("Menu 'Button' Count", "xpath", "//button[@class='ml-0 pl-0 v-btn v-btn--flat theme--light']", ParentTest, "no_jira"); 
                                if (FAIL) { return;}
                        }
                    ...
                    case "Payment ID":  
                        ...
                        break;
                    case "Meal Plan":        
                        if (flag==2) {//Removing Meal plan from station
                           Element_E1_Find("Find Meal Plan", "id", "toc-mealplan", ParentTest, "no_jira"); 
                                if (FAIL) { return;}
    ...
    try {    
        ...
        if(meta.has("hidden")) {  
            ...
            _t++;
            _p++; EX += _t + "\t" + "API > Brand is visible" + "\t" + "-" + "\t" + "Brand is Visible" + "\t" + "PASS" + "\t" + " - " + "\t" + " - " + "\t" + LocalDateTime.now().format(A.A.Time_12_formatter) + "\t" + "no_jira" + "\r\n";
             Log_Html_Result("PASS", "Brand is visible", false, ParentTest.createNode(_t + ". " + "API > Brand is visible"), new Date());           
        }
    ...
    try {    
        ...
        if(digital_wallet.isEmpty() && flag == 1) { // Print pass  Expected result Apple pay and google wallet are not excluded.
            _t++;
            _p++; EX += _t + "\t" + "No digital wallet exclusions - as expected" + "\t" + "-" + "\t" + "-" + "\t" + "PASS" + "\t" + " - " + "\t" + " - " + "\t" + LocalDateTime.now().format(A.A.Time_12_formatter) + "\t" + "no_jira" + "\r\n";
           ...
           for(int l = 0;l <digital_wallet.length(); l++) {
            ...
            Log_Html_Result("FAIL", "Digital wallet exclusions Enabled - not expected", true, ParentTest.createNode(_t + ". " + "Digital wallet exclusions Enabled - not expected"), new Date());
    ...
    try {    
        ...
        if (!json.getBoolean("get_phone_number") && flag == 1) {// Print pass expected not to request phone number
            ...
            Log_Html_Result("FAIL", "Request phone number enabled - not expected", false, ParentTest.createNode(_t + ". " + "Request phone number enabled-  not expected"), new Date());
        }
            ...
            Log_Html_Result("FAIL", "Request phone number Not enabled-  not expected", false, ParentTest.createNode(_t + ". " + "Request phone number Not enabled-  not expected"), new Date());
        }
        ...
        if(json.getJSONObject("logos").has("print") && flag ==1) {
            if(json.getJSONObject("logos").getString("print").contains("https://images")) {
                ...
                Log_Html_Result("FAIL", "KDS Receipt Image Not Saved - Not expected", false, ParentTest.createNode(_t + ". " + "KDS Receipt Image Not Saved - Not expected"), new Date());
            }
        }
    ...
    try {    
        ...
        for (int i = 0; i < Groups.length(); i++) {        
            ...
            if(object.has("name") && object.getString("name").equalsIgnoreCase(site)){
                NEW_SITE_ID = object.getString("id");
                if(object.has("meta")){
                    meta = object.getJSONObject("meta");
                    if (meta.has("sector_name")){
                        sectorName = meta.getString("sector_name");
                        if(sectorName.contentEquals(SITE_SECTOR)) { 
                            _t++;
                            _p++; EX += _t + "\t" + "Found-as expected" + "\t" + "Sector: " + sectorName + "\t" + SITE_SECTOR + "\t" + "PASS" + "\t" + " - " + "\t" + " - " + "\t" + LocalDateTime.now().format(A.A.Time_12_formatter) + "\t" + "no_jira" + "\r\n";
                            Log_Html_Result("PASS", "Found-as expected: " + sectorName + " Expected: " + SITE_SECTOR, false, ParentTest.createNode(_t + ". " + "Verify sector name"), new Date());
                            break;
                        } else {
                            _t++;
                            _f++; EX += _t + "\t" + "Not Found - expected" + "\t" + "Sector : " + sectorName + "\t" + SITE_SECTOR + "\t" + "FAIL" + "\t" + " - " + "\t" + " - " + "\t" + LocalDateTime.now().format(A.A.Time_12_formatter) + "\t" + "no_jira" + "\r\n";
                            Log_Html_Result("FAIL", "Not Found. Actual : " + sectorName + " Expected: " + SITE_SECTOR, true, ParentTest.createNode(_t + ". " + "Verify sector name"), new Date());
                        }   
                    } else {
                        _t++;
                        _f++; EX += _t + "\t" + "Not Found" + "\t" + "No Sector" + "\t" + " - " + "\t" + "FAIL" + "\t" + " - " + "\t" + " - " + "\t" + LocalDateTime.now().format(A.A.Time_12_formatter) + "\t" + "no_jira" + "\r\n"; 
                        Log_Html_Result("FAIL", "No Sector Found. Expected: " + SITE_SECTOR, true, ParentTest.createNode(_t + ". " + "Verify sector name"), new Date());     
                    }
                }
            }  
        } 
    ...
    try {    
        ...
        if (meta.has("app_name")){
            if(meta.getString("app_name").contentEquals(app)) { 
                    _t++;
                    _p++; EX += _t + "\t" + "Found-as expected" + "\t" + "App Name in API: " + meta.getString("app_name") + "\t" + app + "\t" + "PASS" + "\t" + " - " + "\t" + " - " + "\t" + LocalDateTime.now().format(A.A.Time_12_formatter) + "\t" + "no_jira" + "\r\n";
                   Log_Html_Result("PASS", "Found-as expected" + "\t" + "App Name: " + meta.getString("app_name"), false, ParentTest.createNode(_t + ". " + "Verify API App name"), new Date());
                      
            } else {
                _t++;   
                _f++; EX += _t + "\t" + "Not Found" + "\t" + "App Name: " + meta.getString("app_name") + "\t" + app + "\t" + "FAIL" + "\t" + " - " + "\t" + " - " + "\t" + LocalDateTime.now().format(A.A.Time_12_formatter) + "\t" + "no_jira" + "\r\n";
                Log_Html_Result("FAIL", "Not Found" + " > " + "App Name: " + meta.getString("app_name") + " > Expected: " + app, true, ParentTest.createNode(_t + ". " + "Verify API App name"), new Date());
            }   
        }else{
            _t++;
        ...
        if(sector_name.equalsIgnoreCase(SITE_SECTOR)) { // ======================== ????
            _t++;  
            _p++; EX += _t + "\t" + "Found-as expected" + "\t" + "Sector : " + sector_name + "\t" + SITE_SECTOR + "\t" + "PASS" + "\t" + " - " + "\t" + " - " + "\t" + LocalDateTime.now().format(A.A.Time_12_formatter) + "\t" + "no_jira" + "\r\n";
           Log_Html_Result("PASS", "Found-as expected: " + sector_name + " Expected: " + SITE_SECTOR, false, ParentTest.createNode(_t + ". " + "Verify sector name"), new Date());
               
        } else {
            _t++;   
            _f++; EX += _t + "\t" + "Not Found - expected" + "\t" + "Sector : " + sector_name + "\t" + SITE_SECTOR + "\t" + "FAIL" + "\t" + " - " + "\t" + " - " + "\t" + LocalDateTime.now().format(A.A.Time_12_formatter) + "\t" + "no_jira" + "\r\n";
           Log_Html_Result("FAIL", "Not Found. Actual: " + sector_name + " Expected: " + SITE_SECTOR, true, ParentTest.createNode(_t + ". " + "Verify sector name"), new Date());
