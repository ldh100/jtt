    ...
    try {    
        ...
        List_L0("Groups Count", "tagName", "tr", ParentTest, "no_jira");              
            ...
            for (int i = 0; i < L0.size(); i++) {
                Element_Text("Group Data Row " + i + " text", L0.get(i), ParentTest, "no_jira");              
            }        
        ...
        List_L0("Groups Count", "tagName", "tr", ParentTest, "no_jira");              
            ...
            if (_All_data)  {
                ...
                Element_Text("Group Data Last Row text", L0.get(L0.size()- 1), ParentTest, "no_jira");               
            }        
        To_Top("Scroll to Page Top", ParentTest, "no_jira");
            if (FAIL) { return;} 
...
Thread.sleep(5000);          
                ...
                Wait_For_Element_By_Path_Visibility("Wait for Add global menu", "xpath", "//span[normalize-space()='Default JDE Category']", ParentTest, "no_jira");
                    if (FAIL) { return;}
                Element_By_Path_Attribute("Read JDE category assigned", "xpath", "(//div[@class='v-select__selection v-select__selection--comma'])[2]", "textContent", ParentTest, "no_jira");
...
//        if(FAIL){return;}
//    Element_By_Path_Text("Get assigned JDE Category", "xpath", "//input[aria-label='JDE Category']/parent::div", ParentTest, "no_jira");
//        if(FAIL){return;}        
    Element_By_Path_Attribute("Get assigned JDE Category", "xpath", "//input[aria-label='JDE Category']/parent::div", "textContent", ParentTest, "no_jira");
        if(FAIL){return;}        
                if(!(JDE_Category.equals(t)))  { //Print JDE category not updated >> Assign JDE category
