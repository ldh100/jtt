...
class AP3_mm_import_mod extends AP3_GUI{
    ...
    protected void run() { 
        ...
        try{
            ...
            File_Find("Find Global mod export Zip File", dest_dir, Export_File_Name, ParentTest, "no_jira"); 
                if (FAIL) { return;}
            Thread.sleep(1000);
            File_UnZip("Unzip global mod export file ", dest_dir, t, ParentTest, "no_jira");
                if (FAIL) { return;}          
            File_Delete("Delete Exported Zip File", dest_dir,t, ParentTest, "no_jira");
                if (FAIL) { return;}  
                
            Export_File_Name = GL_MENU.trim() + "-global-modifier-groups.xlsx";
            Backup_File_Name = Export_File_Name + "_Backup.xlsx";
            File_Copy("Create Exported File Copy", dest_dir + File.separator + Export_File_Name, dest_dir + File.separator + Backup_File_Name, ParentTest, "no_jira");
                if (FAIL) { return;}             
            
            Excel_Edit = Excel_Convert_PLU(dest_dir,Export_File_Name,"Modifier Groups");
            ...
            Excel_Edit = Excel_Edit_DataRow(dest_dir,Export_File_Name,"Modifier Groups");
            if(Excel_Edit.contains("Error")){
            ...
            Excel_Edit = Excel_New_DataRow(dest_dir,Export_File_Name,"Modifier Groups",valueToWrite);
            if(Excel_Edit.contains("Error")){
            ...
            Element_By_Path_Text_Enter("Upload xlsx file", "id", "globalModsImportInput", dest_dir + File.separator + Export_File_Name, false, ParentTest, "no_jira"); 
                if (FAIL) { return;}            
            Thread.sleep(1000);
            ...
            Element_By_Path_Text_Enter("Upload xlsx file", "xpath", "//div[@class='flex shrink']//input[@id='globalModsImportInput'][@type='file'][@accept='.xlsx']", dest_dir + File.separator + Export_File_Name, false, ParentTest, "no_jira"); 
                if (FAIL) { return;}
            ...
            File_Delete("Delete " + Export_File_Name + " file", dest_dir, Export_File_Name, ParentTest, "no_jira"); 
            File_Copy("Restore Exported File Copy", dest_dir + File.separator + Backup_File_Name, dest_dir + File.separator + Export_File_Name, ParentTest, "no_jira");
                if (FAIL) { return;}  
            ...
            Element_By_Path_Text_Enter("Upload xlsx file", "xpath", "//input[@type='file']", dest_dir + File.separator + Export_File_Name, false, ParentTest, "no_jira"); 
                if (FAIL) { return;}
            Thread.sleep(1000);
            Element_By_Path_Click("Click > 'Publish'", "xpath", "//*[contains(text(),'publish')]", ParentTest, "no_jira");
                if (FAIL) { return;}
            ...
            Element_By_Path_Text_Enter("Upload xlsx file", "xpath", "//div[@class='flex shrink']//input[@type='file']", dest_dir + File.separator + Export_File_Name, false, ParentTest, "no_jira"); 
                if (FAIL) { return;}
            Find_Text("Find Import Errors", "        There were errors while trying to import from your excel sheet.", true, ParentTest, "no_jira");
...
//                if (FAIL) { return;}  
    ...
    public void Delete_Test_Items(){// Delete Imported Test Mod Group >>> "XXXX " + New_ID
        try{            
            File_Delete("Delete " + Backup_File_Name + " file", dest_dir, Backup_File_Name, ParentTest, "no_jira");
    ...
    public String Excel_Convert_PLU(String filePath, String fileName, String sheetName) {
        try{
    ...
    public String Excel_Edit_DataRow(String filePath, String fileName, String sheetName) {
        try {
            File file = new File(filePath + File.separator + fileName);      //Create an object of File class to open xlsx file
            FileInputStream inputStream = new FileInputStream(file);         //Create an object of FileInputStream class to read excel file
            Workbook modifier_Workbook = null;
            ...
            if(rowCount != 1) {       //If modifier group/modifier exist
                for(int i = sheet.getLastRowNum(); i > 1; i--) {
                    ...
                    if(cell.getStringCellValue().equals("Modifier") && flag_modifier != 1) {
                        ...
                        } //row.getCell(11).setCellValue(30);  // java.lang.NullPointerException
    ...
    public String Excel_New_DataRow(String filePath, String fileName, String sheetName, String[] dataToWrite) {
        try{
            File file =    new File(filePath+ File.separator + fileName);           //Create an object of File class to open xlsx file
            FileInputStream inputStream = new FileInputStream(file);     //Create an object of FileInputStream class to read excel file
            Workbook modifier_Workbook = null;
