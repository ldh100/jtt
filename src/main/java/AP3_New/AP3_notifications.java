...
class AP3_notifications extends AP3_GUI{
    ...
    protected void run() throws InterruptedException, Exception {
        ...
        if (!env.equals("PR")) {
            ...
            API_Body_Contains("Notification API - Text",  t,"text", true, ParentTest, "no_jira");    
            API_Body_Contains("Notification API - Date Modified",  t,"date_modified", true, ParentTest, "no_jira");    
            API_Body_Contains("Notification API - Status",  t,"status", true, ParentTest, "no_jira"); 
            API_Body_Contains("Notification API - Release Date",  t,"release_date", true, ParentTest, "no_jira"); 
            // </editor-fold>  
        
            // <editor-fold defaultstate="collapsed" desc="Verify Notifications Page">  
            List_L3("Ap3 Notifications table headers", "xpath", "//*[@role='columnheader']", ParentTest, "no_jira");
                if (FAIL) { return;}
            Element_Text("Notification Name", L3.get(0), ParentTest, "no_jira");
                if (FAIL) { return;}
            Element_Text("Notification Description", L3.get(1), ParentTest, "no_jira");
                if (FAIL) { return;}
            Element_Text("Notification Status", L3.get(2), ParentTest, "no_jira");
                if (FAIL) { return;}
            Element_Text("Date Scheduled", L3.get(3), ParentTest, "no_jira");
                if (FAIL) { return;}
            Element_Text("Action", L3.get(4), ParentTest, "no_jira");
                if (FAIL) { return;}
            Wait_For_Element_By_Path_Presence("Bell icon", "xpath", "(//button[contains(@class, 'bell-button')])[1]", ParentTest, "no_jira");
                if (FAIL) { return;}
        ...
        Element_Child_Click("Click > Date Input Field", L1.get(1), "xpath", "//input[@aria-label='Date']", ParentTest, "no_jira");
            if (FAIL) { return;}
        Wait_For_Element_By_Path_Presence("Date Picker", "xpath", "//div[contains(@class,'v-picker--date')]", ParentTest, "no_jira");
            if (FAIL) { return;}
        Element_Child_Click("Select Toady's Date", L1.get(1), "xpath", "//button[contains(@class,'v-btn--active')]", ParentTest, "no_jira");
            if (FAIL) { return;}
        Wait_For_Element_By_Path_Presence("Calendar Icon", "xpath", "//i[contains(@class,'mdi-calendar')]", ParentTest, "no_jira");
            if (FAIL) { return;}
        ...
        // </editor-fold>
            ...
            List_L3("Get List of Records in Notification Management Table", "xpath", "//tr", ParentTest, "no_jira");
                if (FAIL) { return;}
            Element_Child_Text("Created Notification > Notification Name", L3.get(0), "xpath", "//div[contains(text(),'Auto-notif-test')]", ParentTest, "no_jira");
                if (FAIL) { return;}
            Element_Child_Text("Created Notification > Notification Description", L3.get(0), "xpath", "//div[contains(text(),'Auto-notif-test-desc')]", ParentTest, "no_jira");
                if (FAIL) { return;}
            Element_Child_Text("Created Notification > Notification Status", L3.get(0), "xpath", "//div[contains(text(),'Pending')]", ParentTest, "no_jira");
                if (FAIL) { return;}
            ...
            Element_Child_Text("Created Notification > Date Scheduled", L3.get(0), "xpath", "//span[contains(text(),'"+DATE+"')]", ParentTest, "no_jira");
                if (FAIL) { return;}
            // </editor-fold>
        
            // <editor-fold defaultstate="collapsed" desc="Edit and Cancel"> 
        Element_Child_Click("Click > Edit icon", L3.get(0), "xpath", "//i[contains(@class, 'mdi-pencil')]", ParentTest, "no_jira");
            if (FAIL) { return;}
        Thread.sleep(1000);
        ...
        List_L3("Get List of Records in Notification Management Table", "xpath", "//tr", ParentTest, "no_jira");
            if (FAIL) { return;}
        Element_Child_Text("Canceled edit Notification > Notification Name", L3.get(0), "xpath", "//div[contains(text(),'Auto-notif-test')]", ParentTest, "no_jira");
            if (FAIL) { return;}
        Element_Child_Text("Canceled edit Notification > Notification Description", L3.get(0), "xpath", "//div[contains(text(),'Auto-notif-test-desc')]", ParentTest, "o_jira");
            if (FAIL) { return;}
        Element_Child_Text("Canceled edit Notification > Notification Status", L3.get(0), "xpath", "//div[contains(text(),'Pending')]", ParentTest, "no_jira");
            if (FAIL) { return;}
        Element_Child_Text("Canceled edit Notification > Date Scheduled", L3.get(0), "xpath", "//span[contains(text(),'"+DATE+"')]", ParentTest, "no_jira");
            if (FAIL) { return;}
        // </editor-fold>
        ...
        **/
        ...
        To_Bottom("Scroll to Bottom",  ParentTest, "no_jira");
        Element_By_Path_Text_Enter("Enter Current Time + 1 minute Start Time", "xpath", "//input[@aria-label='Start Time']", updatedTime, false, ParentTest, "no_jira");
            if (FAIL) { return;}
        ...
        Page_URL("AP3 Notification URL", ParentTest, "no_jira");
            if (FAIL) { return;}
        Element_By_Path_Text("Original Notification Name", "xpath", "(//div[contains(text(), 'Released')]/ancestor::tr)[1]/td[1]", ParentTest, "no_jira");
            if (FAIL) { return;}
        String nn = t;
         Element_By_Path_Text("Original Notification Description", "xpath", "(//div[contains(text(), 'Released')]/ancestor::tr)[1]/td[2]", ParentTest, "no_jira");
            if (FAIL) { return;}
        String nd = t;   
        ...
        **/
        ...
        Element_By_Path_Text("Status has Changed from 'Pending' > 'Released'", "xpath", "//div[contains(text(),'Released')]", ParentTest, "no_jira");
            if (FAIL) { return;}
        Element_By_Path_Text("Bell Notif. Number", "xpath", "(//button[contains(@class, 'bell-button')])[1]//span", ParentTest, "no_jira");
            if (FAIL) { return;}
        ...
        Wait_For_Element_By_Path_Presence("Bell Notif. Panel", "xpath", "(//aside[contains(@class,'v-navigation-drawer')])[2]", ParentTest, "no_jira");
            if (FAIL) { return;}
        Element_By_Path_Click("Click > First Notif. Card", "xpath", "(//div[contains(@class,'notification-card')])[1]", ParentTest, "no_jira");
            if (FAIL) { return;}
        Element_By_Path_Text("Updated Bell Notif. Number", "xpath", "(//button[contains(@class, 'bell-button')])[1]//span[contains(text(),'"+String.valueOf(BNN-1)+"')]", ParentTest, "no_jira");
            if (FAIL) { return;}
        // </editor-fold>
